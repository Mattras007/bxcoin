...
This tutorial will describe how to use two instances of Bxcoin Core, one online and one offline, to greatly increase security by not having private keys reside on a networked device.
...
This workflow uses [Partially Signed Bxcoin Transactions](https://github.com/bxcoin/bxcoin/blob/master/doc/psbt.md) (PSBTs) to transfer the transaction to and from the offline wallet for signing using the private keys.
...
## Overview
In this tutorial we have two hosts, both running Bxcoin v25.0
...
```sh
[offline]$ ./src/bxcoin-cli -signet -named createwallet \
...
```sh
[offline]$ ./src/bxcoin-cli -signet -rpcwallet="offline_wallet" listdescriptors \
             | jq -r '.descriptors' \
...
```sh
[online]$ ./src/bxcoin-cli -signet -named createwallet \
...
```sh
[online]$ ./src/bxcoin-cli -signet -rpcwallet="watch_only_wallet" importdescriptors "$(cat /path/to/descriptors.json)"
...
```sh
[online]$ ./src/bxcoin-cli -signet -rpcwallet="watch_only_wallet" getnewaddress
...
```sh
[online]$ ./src/bxcoin-cli -signet -rpcwallet="watch_only_wallet" listunspent
...
```sh
[online]$ ./src/bxcoin-cli -signet -rpcwallet="watch_only_wallet" send \
...
```sh
[offline]$ ./src/bxcoin-cli -signet decodepsbt $(cat /path/to/funded_psbt.txt)

{
    ...
}

[offline]$ ./src/bxcoin-cli -signet analyzepsbt $(cat /path/to/funded_psbt.txt)
...
```sh
[offline]$ ./src/bxcoin-cli -signet -rpcwallet="offline_wallet" walletpassphrase "** enter passphrase **" 60
```
...
2. Process, sign and finalize the PSBT on the `offline_wallet` using the `walletprocesspsbt` command, saving the output to a file `final_psbt.txt`.
 ...
 ```sh
[offline]$ ./src/bxcoin-cli -signet -rpcwallet="offline_wallet" walletprocesspsbt \
...
```sh
[online]$ ./src/bxcoin-cli -signet sendrawtransaction $(cat /path/to/final_psbt.txt)
...
```sh
[online]$ ./src/bxcoin-cli -signet -rpcwallet="watch_only_wallet" getbalances
...
```sh
[online]$ ./src/bxcoin-cli -signet -rpcwallet="watch_only_wallet" listtransactions
