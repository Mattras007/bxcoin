...
Bxcoin Core version 23.0 is now available from:

  <https://bitcoincore.org/bin/bxcoin-core-23.0/>
...
- Before this release, Bxcoin Core had a strong preference to try to connect only to peers that listen on port 8333. As a result of that, Bxcoin nodes listening on non-standard ports would likely not get any Bxcoin Core peers connecting to them. This preference has been removed. (#23542)

- Full support has been added for the CJDNS network. See the new option `-cjdnsreachable` and [doc/cjdns.md](https://github.com/bxcoin/bxcoin/tree/23.x/doc/cjdns.md) (#23077)
...
Bxcoin Core release binaries for Linux now include experimental tracepoints which
...
[doc/tracing.md](https://github.com/bxcoin/bxcoin/blob/23.x/doc/tracing.md) and
usage examples are provided in [contrib/tracing/](https://github.com/bxcoin/bxcoin/tree/23.x/contrib/tracing).
...
- The top-level fee fields `fee`, `modifiedfee`, `ancestorfees` and `descendantfees`
  ...
  fields in the `fees` object are denominated in BX. (#22689)
...
* On startup, the list of banned hosts and networks (via `setban` RPC) in
  `banlist.dat` is ignored and only `banlist.json` is considered. Bxcoin Core
...
- CLI `-addrinfo` now returns a single field for the number of `onion` addresses
  ...
  for Tor V2 addresses was removed from Bxcoin Core in 22.0. (#22544)
