...
Bxcoin Core version 24.0.1 is now available from:

  <https://bitcoincore.org/bin/bxcoin-core-24.0.1/>
...
This version of Bxcoin Core adds a new `mempoolfullrbf` configuration
...
Some Bxcoin services today expect that the first version of an
...
The Bxcoin Protocol does not, and cannot, provide any assurance that
...
fees, was realized in [Bxcoin Core 0.12.0][] (February 2016) with the
...
default for years, and several contributors to Bxcoin Core are
...
[Bxcoin Core 0.12.0]: https://bitcoincore.org/en/releases/0.12.0/#opt-in-replace-by-fee-transactions
[bip125]: https://github.com/bxcoin/bips/blob/master/bip-0125.mediawiki
...
- The `wsh()` output descriptor was extended with Miniscript support. You can import Miniscript
  ...
  the Bxcoin Core wallet yet.
  You can find more about Miniscript on the [reference website](https://bxcoin.sipa.be/miniscript/). (#24148)

- The `tr()` output descriptor now supports multisig scripts through the `multi_a()` and
  `sortedmulti_a()` functions. (#24043)

- To help prevent fingerprinting transactions created by the Bxcoin Core wallet, change output
...
[documentation](https://github.com/bxcoin/bxcoin/blob/master/doc/managing-wallets.md#migrating-legacy-wallets-to-descriptor-wallets).
...
- Configuration changes made in the bxcoin GUI (such as the pruning setting,
...
- Also, the interaction between GUI settings and `bxcoin.conf` settings is
simplified. Settings from `bxcoin.conf` are now displayed normally in the GUI
settings dialog, instead of in a separate warning message ("Options set in this
dialog are overridden by the configuration file: -setting=value"). And these
settings can now be edited because `settings.json` values take precedence over
`bxcoin.conf` values. (#15936)
