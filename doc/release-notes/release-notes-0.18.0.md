Bxcoin Core version 0.18.0 is now available from:

  <https://bitcoincore.org/bin/bxcoin-core-0.18.0/>
...
`/Applications/Bxcoin-Qt` (on Mac) or `bitcoind`/`bxcoin-qt` (on
Linux).
...
recommended to use Bxcoin Core on unsupported systems.

Bxcoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Bxcoin Core does not yet change appearance when
...
`~bxcoin/.bxcoin`. When switching over to the new configuration file,
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/bxcoin/bxcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the Bxcoin Core
...
`~bxcoin/.bxcoin`. This change makes Bxcoin Core more consistent with
...
`datadir` option in `/etc/bxcoin/bxcoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/bxcoin/bxcoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/bxcoin/bxcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/bxcoin/bxcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/bxcoin/bxcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/bxcoin/bxcoin/blob/master/doc/bxcoin-conf.md)
  about the `bxcoin.conf` file describes how to use it to configure
  Bxcoin Core.

- A new document introduces Bxcoin Core's BIP174 [Partially-Signed
  Bxcoin Transactions
  (PSBT)](https://github.com/bxcoin/bxcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/bxcoin/bxcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Bxcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Bxcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/bxcoin/bxcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/bxcoin/bxcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Bxcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Bxcoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/bxcoin/bxcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/bxcoin/bxcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- A new `bxcoin-wallet` tool is now distributed alongside Bxcoin
...
This section describes planned changes to Bxcoin Core that may affect
other Bxcoin software and services.

- Since version 0.16.0, Bxcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Bxcoin Core 0.20 (expected about
  a year after 0.18), Bxcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Bxcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Bxcoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Bxcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Bxcoin Core
...
- A sub-project of Bxcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Bxcoin Core.  See their [project
  page](https://github.com/bxcoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Bxcoin Core's own implementation, although entropy
  gathered by Bxcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Bxcoin Core a
...
- On macOS, Bxcoin Core now opts out of application CPU throttling
...
- #13926 [Tools] bxcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14521 qt, docs: Fix `bxcoin-qt -version` output formatting (hebasto)
...
- #15154 Configure: bxcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/bxcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Bxcoin Core (sipsorcery)
...
- #13953 Fix deprecation in bxcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Bxcoin Core (Christewart)
...
- #12246 Bugfix: Only run bxcoin-tx tests when bxcoin-tx is enabled (luke-jr)
...
- #14186 bxcoin-cli: don't translate command line options (HashUnlimited)
...
- #12255 Update bxcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/bxcoin-conf.md (hebasto)
...
- #15354 Add missing `bxcoin-wallet` tool manpages (MarcoFalke)
...
- #15802 mention creating application support bxcoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/bxcoin/).
