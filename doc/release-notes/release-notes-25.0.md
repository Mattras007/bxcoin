...
Bxcoin Core version 25.0 is now available from:

  <https://bitcoincore.org/bin/bxcoin-core-25.0/>
...
- All JSON-RPC methods accept a new [named
  parameter](https://github.com/bxcoin/bxcoin/blob/master/doc/JSON-RPC-interface.md#parameter-passing) called `args` that can
  contain positional parameter values. This is a convenience to allow some
  parameter values to be passed by name without having to name every value. The
  python test framework and `bxcoin-cli` tool both take advantage of this, so
  for example:

```sh
bxcoin-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
bxcoin-cli -named createwallet mywallet load_on_startup=1
```
...
dbcache, Bxcoin Core will now return an error at startup. (#25574)
...
before Bxcoin Core has begun its shutdown sequence. (#23395)
...
- The binary verification script has been updated. In previous releases it
  ...
  examples, see:
  https://github.com/bxcoin/bxcoin/blob/master/contrib/verify-binaries/README.md
  (#27358)
