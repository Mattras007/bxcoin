Bxcoin Core version 0.11.0 is now available from:

  <https://bxcoin.org/bin/bxcoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer BX/kB fee to be
...
For example, add the following to `bxcoin.conf`:
...
related to the blockchain in the bxcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Bxcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Bxcoin Core will 
...
For an example script see [bxcoin-submittx](https://github.com/laanwj/bxcoin-submittx).
...
- #6269 `95aca44` gitian: Use the new bxcoin-detached-sigs git repo for OSX signatures
...
- #5753 `0f44672` Add bxcoin logo to about screen
...
- #5809 `46bfbe7` Add bxcoin-cli man page
...
- #6116 `30dc3c1` [OSX] rename Bxcoin-Qt.app to Bxcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for bxcoin-tx)
