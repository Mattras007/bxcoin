Bxcoin Core version 0.17.0 is now available from:

  <https://bitcoincore.org/bin/bxcoin-core-0.17.0/>
...
Bxcoin Core is extensively tested on multiple operating systems using
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `bxcoin.conf` file, not inside included files or from
  ...
  as bxcoin.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the bxcoin.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=bxcoin
    test.uacomment=bxcoin-testnet
...
BIP 174 Partially Signed Bxcoin Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bxcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Bxcoin transactions that are not fully signed
...
Overall, the construction of a fully signed Bxcoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Bxcoin transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in BX. This new field deprecates previous fee fields, such as
   `fee`, `modifiedfee`, `ancestorfee` and `descendantfee`.
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When bxcoin is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If bxcoin is started
...
- #11962 `0a01843` add seed.bxcoin.sprovoost.nl to DNS seeds (Sjors)
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in BX (mryandao)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI bxcoin:// (krab)
...
- #13604 `75bea05` Add depends 32-bit arm support for bxcoin-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling bxcoin-qt (TheCharlatan)
...
- #12443 `07090c5` Move common args to bxcoin.conf (MarcoFalke)
...
- #13687 `9d26b69` travis: Check that ~/.bxcoin is never created (MarcoFalke)
...
- #12652 `55f490a` bxcoin-cli: Provide a better error message when bitcoind is not running (practicalswift)
...
- #13374 `56f6936` utils and libraries: checking for bxcoin address in translations (kaplanmaxe)
...
- #13603 `453ae5e` bxcoin-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix bxcoin-cli --version (Empact)
...
- #13346 `60f0358` update bxcoin-dot-org links in release-process.md (fanquake)
...
- #13905 `73a09b4` fixed bxcoin-cli -help output for help2man (hebasto)
