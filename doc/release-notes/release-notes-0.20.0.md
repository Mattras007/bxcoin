...
Bxcoin Core version 0.20.0 is now available from:

  <https://bitcoincore.org/bin/bxcoin-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Bxcoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Bxcoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Bxcoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Bxcoin Core. Bxcoin Core logs debug messages
...
- OpenSSL is no longer used by Bxcoin Core.  (#17265)

- BIP70 support has been fully removed from Bxcoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/bxcoin/bxcoin/issues/16599),
  [PR #16702](https://github.com/bxcoin/bxcoin/pull/16702), and the
...
- All custom settings configured when Bxcoin Core starts are now
...
- The "Start Bxcoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple Bxcoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Bxcoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bxcoin-core/HWI) for
  signing.  Future versions of Bxcoin Core should support a GUI option
...
- Bxcoin Core's automatically-generated source code documentation is
...
- The `bxcoin-cli` utility used with the `-getinfo` parameter now
...
- The `bxcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #18702 Fix ASLR for bxcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate bxcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bxcoin.conf (sdaftuar)
...
- #18166 ci: Run fuzz testing test cases (bxcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone bxcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Bxcoin Core files (hebasto)
...
- #17119 Fix broken bxcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from bxcoin-qt help message and manpage (laanwj)
...
- #17648 Rename wallet-tool references to bxcoin-wallet (hel-o)
...
- #18341 Replace remaining literal BX with `CURRENCY_UNIT` (domob1812)
...
- #17650 util: Remove unwanted fields from bxcoin-cli -getinfo (malevolent)
