# User-space, Statically Defined Tracing (USDT) for Bxcoin Core

Bxcoin Core includes statically defined tracepoints to allow for more
...
**Note**: Bxcoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Bxcoin Core
...
To list probes in Bxcoin Core, use `info probes` in `gdb`:
...
The `readelf` tool can be used to display the USDT tracepoints in Bxcoin Core.
