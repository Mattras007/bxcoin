# PSBT Howto for Bxcoin Core

Since Bxcoin Core 0.17, an RPC interface exists for Partially Signed Bxcoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/bxcoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Bxcoin transactions that are not fully signed
...
#### Multisig with multiple Bxcoin Core instances
...
Bxcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bxcoin-cli` is used.
...
Later, when *V* BX has been received on *Amulti*, and Bob and Carol want to
