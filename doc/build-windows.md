...
Below are some notes on how to build Bxcoin Core for Windows.

The options known to work for building Bxcoin Core on Windows are:
...
Note that for WSL the Bxcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/bxcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\bxcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/bxcoin
