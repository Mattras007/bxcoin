# CJDNS support in Bxcoin Core

It is possible to run Bxcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Bxcoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Bxcoin network and individual nodes.
...
before it will work with your Bxcoin Core node. This is described in steps
...
## Run Bxcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Bxcoin Core
...
When enabled, this option tells Bxcoin Core that it is running in an
...
IPv6 local network. This helps Bxcoin Core perform better address management:
...
CJDNS support was added to Bxcoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bxcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/bxcoin/bxcoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Bxcoin Core

There are several ways to see your CJDNS address in Bxcoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bxcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bxcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `bxcoin-cli help getnodeaddresses` for details.
