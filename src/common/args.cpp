...
const char * const BITCOIN_CONF_FILENAME = "bxcoin.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bxcoin-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Bxcoin
    // macOS: ~/Library/Application Support/Bxcoin
    // Unix-like: ~/.bxcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bxcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Bxcoin";
#else
    // Unix-like
    return pathRet / ".bxcoin";
#endif
#endif
}
