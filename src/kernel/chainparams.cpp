...
// Copyright (c) 2009-2021 The Bxcoin Core developers
...
public:
    CMainParams() {
        ...
        // Consensus
        consensus.nSubsidyHalvingInterval = 100000;  // halving every 100k blocks (~138 days)
        consensus.BIP34Height = 1;
        consensus.BIP34Hash = uint256();  // no predefined hash yet
        consensus.BIP65Height = 1;   // OP_CHECKLOCKTIMEVERIFY enabled from block 1
        consensus.BIP66Height = 1;   // Strict DER enabled from block 1
        consensus.CSVHeight = 1;     // CheckSequenceVerify enabled early
        consensus.SegwitHeight = 1;  // Segwit enabled early
        consensus.MinBIP9WarningHeight = 2;  // Warning if no activation after this height

        consensus.powLimit = uint256S("00000fffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"); // easier than Bitcoin mainnet
        consensus.nPowTargetTimespan = 14 * 24 * 60 * 60; // two weeks difficulty adjustment window
        consensus.nPowTargetSpacing = 2 * 60;  // 2 minutes block time

        consensus.fPowAllowMinDifficultyBlocks = false; // no testnet style min difficulty
        consensus.fPowNoRetargeting = false;            // enable retargeting
        ...
        // Network magic bytes (change from Bitcoin)
        pchMessageStart[0] = 0x42; // B
        pchMessageStart[1] = 0x58; // X
        pchMessageStart[2] = 0x43; // C
        pchMessageStart[3] = 0x4E; // N

        nDefaultPort = 9333; // Bxcoin default port
        nPruneAfterHeight = 100000;

        m_assumed_blockchain_size = 1;   // MB placeholder
        ...
        // assert(consensus.hashGenesisBlock == uint256S("your_generated_hash_here"));

        // Seeds empty for now
        vSeeds.clear();

        // Base58 prefixes (example, customize as needed)
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1, 60);  // 'M' (example)
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1, 122); // 'm' (example)
        base58Prefixes[SECRET_KEY] = std::vector<unsigned char>(1, 128);
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};  // testnet style, change if needed
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        bech32_hrp = "bx"; // Bxcoin HRP

        vFixedSeeds.clear(); // clear fixed seeds for now
        ...
        checkpointData = {
            {
                // No checkpoints yet — add later as chain matures
            }
        };
...
public:
    explicit SigNetParams(const SigNetOptions& options)
    {
        ...
        if (!options.challenge) {
            ...
            vSeeds.emplace_back("seed.signet.bxcoin.sprovoost.nl.");
...
public:
    explicit CRegTestParams(const RegTestOptions& opts)
    {
        ...
        checkpointData = {
            {
                {0, uint256S("000003950cda92cedc6f9e0729531a8fae870795d29117adfbcc47cc9dc5eb33")},
            }
        };
