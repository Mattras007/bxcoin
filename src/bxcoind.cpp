...
static bool ParseArgs(ArgsManager& args, int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        if (!IsSwitchChar(argv[i][0])) {
            return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see bxcoind -h for a list of options.", argv[i])));
        }
    }
    return true;
}
...
static bool ProcessInitCommands(ArgsManager& args)
{
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  bxcoind [options]                     Start " PACKAGE_NAME "\n"
                "\n";
...
static bool AppInit(NodeContext& node)
{
    ...
    try
    {
        // -server defaults to true for bxcoind but not for the GUI so do this here
...
#if HAVE_DECL_FORK
            tfm::format(std::cout, "Bxcoin Core is starting\n");
...
MAIN_FUNCTION
{
    ...
    // Connect bxcoind signal handlers
    noui_connect();
