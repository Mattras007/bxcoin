...
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "$bxcoin_qt_want_version" = "auto" && test "$bxcoin_qt_force" != "yes"; then
    if test "$bitcoin_enable_qt" != "no"; then
      AC_MSG_WARN([$1; bxcoin-qt frontend will not be built])
    fi
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "$bitcoin_enable_qt" != "no" && test "$bxcoin_qt_want_version" != "no"; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build bxcoin-qt GUI (default=auto)])],
    [
     bxcoin_qt_want_version=$withval
     if test "$bxcoin_qt_want_version" = "yes"; then
       bxcoin_qt_force=yes
       bxcoin_qt_want_version=auto
     fi
    ],
    [bxcoin_qt_want_version=auto])
  ...
  AS_IF([test "$with_gui" = "qt5_debug"],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); bxcoin_qt_want_version=qt5],
        [qt_lib_suffix= ])
