# Bootstrappable Bxcoin Core Builds

This directory contains the files necessary to perform bootstrappable Bxcoin
Core builds.
...
To build Bxcoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Bxcoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/bxcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/bxcoin-core/guix.sigs).
